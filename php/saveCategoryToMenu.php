...
$result00 = mysql_query(sprintf("SELECT configs FROM menutype where company=%s",$company->company));
if($result00){
    ...
    if(in_array($cat->id,$comfigsArr)){
        ...
        $query = sprintf("UPDATE categories SET category='%s',folder='%s',img='%s' "
                ...
                json_encode($cat->catj),$cat->folder,$pic,$cat->id,$company->company);
