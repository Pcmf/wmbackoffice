...
$result0 = mysql_query(sprintf("SELECT * FROM menutype WHERE company = %s",$parm->id));
if($result0){
   ...
   //if the company don't have yet a menu then load the menu model for his business from business type
    $result = mysql_query(sprintf("SELECT * FROM business WHERE id= %s",$parm->businesstype));
    if($result){
...
function getCats($configs,$parm){
    ...
    foreach ($catArr as $value) {
       $result00 = mysql_query(sprintf("SELECT * FROM categories WHERE id =%s AND company=%s",$value,$parm->id));
       if($result00){
