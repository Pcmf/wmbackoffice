...
angular.module('WMbackoffice').controller('productsController',function($scope,$modal,$http,$rootScope){
  ...
  $http({
                ...
                for(i=0; i < k.length ; i++){
                    ...
                    k[i].className = k[i].className.replace(" show", "");
                        ...
                        if (x.className.indexOf("show") == -1) {
                            ...
                            var params = {};
                            params.cid = JSON.parse(sessionStorage.company).id;
                   //         alert(params.cid);
                            params.cat = cat;
     ...
     //Function to ADD a NEW Product       
            $scope.addNewProduct = function(){
             ...
             prd.id =0;
             prd.catid = cat;
             prd.cid = JSON.parse(sessionStorage.company).id;
                // function to EDIT the products ON MENU
                    var modalInstance1 = $modal.open({
                        templateUrl: 'modalEditProduct.html',
                        controller: 'modalInstanceEditProd',
...
//    console.log(items);
    ...
    $scope.saveCommunProd = function(prd){
        var parm ={};
        parm.cid = JSON.parse(sessionStorage.company).id;
        parm.prd = prd;
